...
# Hide or show window decorations using Tkinter
manager = plt.get_current_fig_manager()
decoration_state = True  # Window decorations are on by default

if hasattr(manager, 'window'):
    root = manager.window
    if isinstance(root, Tk):
        def toggle_decorations(event=None):
            """Toggle window decorations on F5 key press."""
            global decoration_state
            decoration_state = not decoration_state
            root.overrideredirect(not decoration_state)

        # Bind the F5 key to toggle decorations
        root.bind('<F5>', toggle_decorations)
...
def draw_text_with_box(ax, x, y, text, fontsize=9, color="white", box_color="black", alpha=0.5):
    return ax.text(
        x,
        y,
        text,
...
def update(frame):
    ...
    draw_text_with_box(ax, 0.5, 0.8, f"Total Packets Sent: {total_packets}")
    ...
    draw_text_with_box(ax, 0.5, 0.20, f"Irregular Latencies: {irregular_latency_count}")
